<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Monitoring Absensi Worker (Level 3)
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-4">
                    <form method="GET" class="flex flex-wrap gap-4 items-end">
                        ...
                        <div>
                            <label for="search" class="block text-sm font-medium text-gray-700">Cari Nama Worker</label>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Worker</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($att->photo_in_path)
                                            <a href="{{ Storage::url($att->photo_in_path) }}" target="_blank">
                                                <img src="{{ Storage::url($att->photo_in_path) }}" alt="Foto selfie" class="h-10 w-10 rounded-full object-cover border border-gray-200 cursor-pointer hover:opacity-75">
                                            </a>
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div>{{ $att->location ?? '-' }}</div>
                                        @if($att->latitude && $att->longitude)
                                            <a href="https://www.google.com/maps?q={{ $att->latitude }},{{ $att->longitude }}" target="_blank" class="text-indigo-600 hover:text-indigo-900 text-xs">
                                                Lihat GPS
                                            </a>
                                        @endif
                                    </td>
                                ...
                                @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-8 text-center text-gray-500">Tidak ada data absensi untuk Worker pada tanggal ini.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
